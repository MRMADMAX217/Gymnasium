...
def run_training(episodes, render=False):
    ...
    for i in range(episodes):
        ...
        while(not terminated and not truncated):
            ...
            # Choose next action (SARSA is on-policy, so we need a' for the update)
            if rng.random() < epsilon:
    ...
    plt.savefig('sarsa_frozen_lake_training.png')
    print("Plot saved to sarsa_frozen_lake_training.png")

    # Save Q-table
    f = open("sarsa_q_table.pkl","wb")
    pickle.dump(q, f)
    f.close()
    print("Q-table saved to sarsa_q_table.pkl")

if __name__ == '__main__':
    run_training(40000, render=False)
