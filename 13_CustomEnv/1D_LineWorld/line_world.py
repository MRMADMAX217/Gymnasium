...
class LineWorldEnv(gym.Env):
    ...
    def _render_frame(self):
        if self.window is None and self.render_mode == "human":
            pygame.init()
            pygame.display.init()
            self.window = pygame.display.set_mode((self.window_size, self.window_size / 5))
        
        if self.clock is None and self.render_mode == "human":
            self.clock = pygame.time.Clock()

        canvas = pygame.Surface((self.window_size, self.window_size / 5))
...
if __name__ == "__main__":
    ...
    while not (terminated or truncated):
        ...
        print(f"Iteration: {i}, Action: {action}, Obs: {obs}, Reward: {reward}, Termi: {terminated}, Trunc: {truncated}")
        i+=1    
    env.close()
